...
export default function DonationFinder() {
  ...
  const [contactList, setContactList] = useState([]); // saved place names
  const [error, setError] = useState(null);

  const destLabel = DESTINATIONS.find((d) => d.id === destination)?.label ?? "a shelter";
  ...
  async function callClaude(promptText, useSearch = false) {
    const body = {
  ...
  async function handleSort() {
    ...
    setStep("sorting");
    try {
      const itemsText = itemList.map((i) => `- ${i.name} (${i.condition === "new" ? "new / unused" : "used, good condition"})`).join("\n");
      const prompt = `Someone wants to donate belongings to: ${destLabel}. ${
        context ? `Context: ${context}.` : ""
      } Here are their items with condition:
...
[{"item": "string (echo the item name exactly)", "condition": "new|used", "category": "donate|keep|dispose", "reason": "one short sentence, gentle and practical"}]`;
  ...
  async function handleFindPlaces() {
    ...
    setError(null);
    try {
      const donateItems = sorted.filter((i) => i.category === "donate");
      const donateNames = donateItems.map((i) => `${i.item} (${i.condition === "new" ? "new" : "used"})`);
      ...
      const prompt = `Search the web for organizations near "${location}" that fit this donation destination: ${destLabel}, and that currently accept donations of these items: ${donateNames.join(
        ", "
...
Return at most 6 results.`;
  ...
  function toggleContact(placeName) {
    setContactList((prev) =>
      prev.includes(placeName) ? prev.filter((n) => n !== placeName) : [...prev, placeName]
    );
  }
  ...
  const savedPlaces = places ? places.filter((p) => contactList.includes(p.name)) : [];
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "48px 24px 80px" }}>
        ...
        {grouped && (step === "sorted" || step === "searching" || step === "done") && (
          <div style={{ marginTop: 8 }}>
            ...
            {step === "done" && savedPlaces.length > 0 && (
              <div style={{ marginBottom: 28, background: "#EEF3EC", borderRadius: 12, padding: "16px 18px", border: "1px solid #CBDAC6" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                  ...
                  <span style={{ fontFamily: "'Helvetica Neue', sans-serif", fontSize: 12, letterSpacing: "0.1em", textTransform: "uppercase", fontWeight: 600, color: "#3D5A45" }}>
                    Your contact list ({savedPlaces.length})
                  </span>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {savedPlaces.map((s, idx) => (
            ...
            {step === "done" && places && (
              <div style={{ marginTop: 4 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {places.map((s, idx) => {
                    const saved = contactList.includes(s.name);
                    return (
